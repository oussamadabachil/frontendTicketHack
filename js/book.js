    ...
    if (data.length) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        document.querySelector(
          ".cart-list"
        ).innerHTML += `<li class="list-group-item"><span>${data[i].departure} > ${data[i].arrival}</span><span>${hourAndMinuteOnly} </span><span>${data[i].price}€</span>Departure in ${timeLeftForTheTrip}<span></span></li> `;
      }
    } else {
    }
  });
